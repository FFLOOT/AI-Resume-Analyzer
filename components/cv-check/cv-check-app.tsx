...
import { LogOut, Scan, LayoutList, Target, Heart, MessageSquareText, Plus, ChevronDown, User, Settings, Sparkles, ArrowRight, Check } from "lucide-react"
...
export function CVCheckApp() {
  ...
  const handleAnalyze = () => {
    if (!cvFile || (!jobFile && !jobText)) return
    ...
    setTimeout(() => {
      ...
      const newHistoryItem: ScanHistoryItem = {
        ...
        jobTitle: jobFile?.name.replace(/\.[^/.]+$/, "") || "Pasted Job Description",
  ...
  const canAnalyze = cvFile && (jobFile || jobText.trim().length > 50)
